...
STUDIO_KNOWLEDGE = """
Информация о приеме и массаже TOCHKA (Батуми):
...
- У меня доступны ИСКЛЮЧИТЕЛЬНО 4 вида массажа тела. Массажа в 4 руки, массажа лица и отдельных зон нет.
...
def load_dynamic_ai_config() -> Dict[str, Any]:
    ...
    elif "База знаний:" in base_prompt or "База знаний студии:" in base_prompt:
...
def clean_ai_response(text: str) -> str:
    ...
    and guarantees 1st-person phrasing without the word 'студия'.
    """
    if not text:
        return ""
    ...
    final_text = result if result else text.strip()

    # 3. Guaranteed safety filters: 1st person & eliminate word 'студия'
    substitutions = [
        (r"(?i)\bадминистратор\s+свяжется\s+с\s+вами\b", "я свяжусь с вами"),
        (r"(?i)\bадминистратор\s+свяжется\b", "я свяжусь"),
        (r"(?i)\bнаш\s+администратор\b", "я"),
        (r"(?i)\bадминистратора\b", "меня"),
        (r"(?i)\bадминистратору\b", "мне"),
        (r"(?i)\bадминистратором\b", "со мной"),
        (r"(?i)\bадминистратор\b", "я"),
        (r"(?i)\bнаш\s+мастер\b", "я"),
        (r"(?i)\bнашему\s+мастеру\b", "мне"),
        (r"(?i)\bнашего\s+мастера\b", "меня"),
        (r"(?i)\bнашим\s+мастером\b", "со мной"),
        (r"(?i)\bнаши\s+мастера\b", "я"),
        (r"(?i)\bнаших\s+мастеров\b", "меня"),
        (r"(?i)\bв\s+нашей\s+студии\b", "у меня в TOCHKA"),
        (r"(?i)\bв\s+студии\s+TOCHKA\b", "у меня в TOCHKA"),
        (r"(?i)\bв\s+студии\b", "у меня в TOCHKA"),
        (r"(?i)\bиз\s+студии\b", "из TOCHKA"),
        (r"(?i)\bстудия\s+TOCHKA\b", "TOCHKA"),
        (r"(?i)\bстудии\s+TOCHKA\b", "TOCHKA"),
        (r"(?i)\bстудию\s+TOCHKA\b", "TOCHKA"),
        (r"(?i)\bстудией\s+TOCHKA\b", "TOCHKA"),
        (r"(?i)\bстудия\b", "пространство"),
        (r"(?i)\bстудии\b", "пространства"),
        (r"(?i)\bстудию\b", "пространство"),
        (r"(?i)\bстудией\b", "пространством"),
        (r"(?i)\bстудиях\b", "пространствах"),
        (r"(?i)\bстудиям\b", "пространствам"),
        (r"(?i)\bв\s+нашем\s+салоне\b", "у меня в TOCHKA"),
        (r"(?i)\bв\s+салоне\b", "у меня в TOCHKA"),
        (r"(?i)\bнаш\s+салон\b", "TOCHKA"),
        (r"(?i)\bмы\s+свяжемся\b", "я свяжусь"),
        (r"(?i)\bмы\s+будем\s+рады\b", "я буду рада"),
        (r"(?i)\bбудем\s+рады\b", "буду рада"),
        (r"(?i)\bбудем\s+счастливы\b", "буду счастлива"),
        (r"(?i)\bждем\s+вас\b", "жду вас"),
        (r"(?i)\bждём\s+вас\b", "жду вас"),
    ]
...
class AIAssistantService:
    ...
    def process_incoming_message(
        ...
        if not ai_cfg.get("ai_enabled", True):
            return {
                "success": True,
                "response_text": "Здравствуйте! 🌿 В данный момент автоответчик отключен. Вы можете написать мне напрямую в WhatsApp или позвонить: +995 591 226 145, либо выбрать время на сайте tochkabatumi.ge ✨",
                "session_id": session_id,
                "channel": channel,
                "tools_called": [],
...
"""
        ...
        if not reply_text:
            reply_text = (
                "Здравствуйте! 🌿 Извините, сейчас связь немного затруднена. "
                "Вы можете сразу написать мне напрямую в WhatsApp или позвонить: +995 591 226 145, либо выбрать время на сайте tochkabatumi.ge ✨"
            )
            success = False
        else:
            success = True
